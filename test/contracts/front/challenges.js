...
describe('Challenges Contract', () => {
  ...
  const getRequestCreationTime = async (challengeId) => challenges.methods.getRequestCreationTime(challengeId).call();
  ...
  const getRequestedBundle = async (challengeId) => challenges.methods.getRequestedBundle(challengeId).call();
  ...
  const requestIsInProgress = async (challengeId) => challenges.methods.requestIsInProgress(challengeId).call();
  const canResolve = async (resolver, challengeId) => challenges.methods.canResolve(resolver, challengeId).call();
  const getRequestDesignatedShelterer = async (challengeId) => challenges.methods.getRequestDesignatedShelterer(challengeId).call();
  ...
  describe('Starting system challenges', () => {
    ...
    it('Stores challengerId as 0x0', async () => {
      await startChallengeForSystem(uploader, bundleId, SYSTEM_CHALLENGES_COUNT, context, systemChallengeFee);
      expect(await getRequestCreationTime(systemChallengeId)).to.equal(now.toString());
    ...
    describe('Stores system challenges correctly', () => {
      ...
      it('Bundle id', async () => {
        expect(await getRequestedBundle(challengeId)).to.equal(bundleId);
      });
      ...
      it('Challenge setup time', async () => {
        expect(await getRequestCreationTime(challengeId)).to.equal(challengeBlockTimestamp.toString());
      });
      ...
      it('Challenge in progress', async () => {
        expect(await requestIsInProgress(challengeId)).to.equal(true);
      });
    });
  });
  ...
  describe('Starting user challenge', () => {
    ...
    it('Challenge is not in progress until started', async () => {
      expect(await requestIsInProgress(challengeId)).to.equal(false);
    });
  ...
  describe('Resolving a challenge', () => {
    beforeEach(async () => {
      ...
      resolver = await getRequestDesignatedShelterer(challengeId);
    });
  ...
  describe('Resolving system challenge', () => {
    ...
    it('Increases sequence number for all resolutions but last', async () => {
      ...
      const anotherResolver = await getRequestDesignatedShelterer(challengeId);
  ...
  describe('Marking system challenge as expired', () => {
    ...
    it(`Returns fee to creator (part of the fee if partially resolved)`, async () => {
      ...
      resolver = await getRequestDesignatedShelterer(systemChallengeId);
    ...
    it(`Deletes challenge with active count bigger than 1`, async () => {
      ...
      expect(await requestIsInProgress(systemChallengeId)).to.equal(false);
    });
  });
  ...
  describe('Designated shelterer testing', () => {
    ...
    const getCurrentRound = async (challengeId) => {
      const creationTime = await getRequestCreationTime(challengeId);
    ...
    it('Fails to get resolver if no resolver in the system', async () => {
      await expect(getRequestDesignatedShelterer(challengeId)).to.be.eventually.rejected;
    });
  });
});
